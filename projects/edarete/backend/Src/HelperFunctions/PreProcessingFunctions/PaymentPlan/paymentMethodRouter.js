...
const ManualAutoRenewalHandler = require("../../PostProcessingFunctions/PaymentPlan/handlers/ManualAutoRenewalHandler");
const GatewayHandler = require("../../PostProcessingFunctions/PaymentPlan/handlers/GatewayHandler");
...
async function paymentMethodRouter(req, decryptedPayload) {
  ...
  try {
    ...
    try {
      ...
      const paymentMethod = await executeQuery(paymentMethodQuery, [payment_method_id, urdd_id], connection);
...
async function validatePaymentMethodForRegion(paymentMethodId, urdd_id, plan_id) {
  ...
  try {
    ...
    const result = await executeQuery(query, [plan_id, paymentMethodId], connection);
